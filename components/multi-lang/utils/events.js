...
const disableParents = function disableParents(e) {
  ...
  for (const parent of parents) {
    // Disable click
    if (parent.style) parent.style.pointerEvents = "none"; // Disable key press

    parent.onkeypressbkp = parent.onkeypress;

    parent.onkeypress = e => {// e.preventDefault()
    };
  }
  ...
  function enableParents(e) {
    for (const parent of parents) {
      // Enable click
      if (parent.style) parent.style.pointerEvents = "all"; // Enable key press
